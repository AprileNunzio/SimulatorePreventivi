...
const PreventivoForm = {
  async renderForm(el, prev) {
    const isNew = !prev;
    let defaultCondPag = '';
    
    if (isNew) {
      try {
        const imp = await window.electronAPI.getImpostazioni();
        defaultCondPag = imp.condizioni_pagamento_default || "Pagamento previsto in due tranche: 50% a titolo di acconto alla firma del contratto e 50% a saldo a completo completamento delle attività.";
      } catch (e) {
        defaultCondPag = "Pagamento previsto in due tranche: 50% a titolo di acconto alla firma del contratto e 50% a saldo a completo completamento delle attività.";
      }
    }
    
    const dataEmissioneDefault = prev?.data_creazione || new Date().toISOString().split('T')[0];
    const dataScadenzaDefault = prev?.data_scadenza || addDays(dataEmissioneDefault, 7);
    el.innerHTML = `
      ...
      <div style="padding:24px 32px;max-width:900px">
        <div class="card" style="margin-bottom:20px">
          ...
          <div class="form-row cols-3">
            ...
            <div class="form-group">
              ...
              <input type="date" class="form-input" id="f-data" value="${dataEmissioneDefault}">
            </div>
            <div class="form-group">
              <label class="form-label">Data Scadenza (validità)</label>
              <input type="date" class="form-input" id="f-scadenza" value="${dataScadenzaDefault}">
              <div class="form-hint">Impostata automaticamente a 7 giorni dalla data di emissione, modificabile.</div>
            </div>
          </div>
          <div class="form-row cols-3">
            <div class="form-group">
        ...
        <div class="card" style="margin-bottom:20px; position: relative; overflow: visible;">
          ...
          <div class="form-row cols-3">
            ...
            <div class="form-group">
              ...
              <input type="text" class="form-input" id="f-cprov" value="${prev?.cliente_provincia||''}" placeholder="MI" maxlength="2">
            </div>
          </div>
          <div class="form-row cols-3">
            <div class="form-group">
              <label class="form-label">Nazione</label>
              <input type="text" class="form-input" id="f-cnaz" value="${prev?.cliente_nazione||'IT'}" placeholder="IT" maxlength="2">
            </div>
            <div class="form-group">
              <label class="form-label">Codice Destinatario (SDI)</label>
              <input type="text" class="form-input" id="f-csdi" value="${prev?.cliente_codice_destinatario||''}" placeholder="0000000" maxlength="7">
            </div>
            <div class="form-group">
              <label class="form-label">PEC</label>
              <input type="email" class="form-input" id="f-cpec" value="${prev?.cliente_pec||''}" placeholder="cliente@pec.it">
            </div>
          </div>
          <div class="form-hint" style="margin-top:-8px;margin-bottom:4px">Servono per generare la fattura elettronica (XML FatturaPA): basta il Codice Destinatario oppure la PEC.</div>
          <div class="form-group">
            <label class="form-label" style="display:flex;align-items:center;gap:10px;">
              Cliente Pubblica Amministrazione
              <label class="switch"><input type="checkbox" id="f-cpa" ${prev?.cliente_pa ? 'checked' : ''}><span class="slider"></span></label>
            </label>
          </div>
        </div>
        ...
        <div class="card" style="margin-bottom:20px">
          ...
          <div class="form-group">
            ...
            <textarea class="form-textarea" id="f-condpag" placeholder="Es. Bonifico a 30 giorni dalla data fattura">${prev?.condizioni_pagamento || defaultCondPag}</textarea>
          </div>
          <div class="form-group">
    ...
    import('../../utils.js').then(({ Snippet }) => {
      Snippet.init('f-notecl', 'preventivo');
      Snippet.init('f-condpag', 'preventivo');
      Snippet.init('f-notein', 'preventivo');
    });

    el.querySelector('#btn-save-prev')?.addEventListener('click', async () => {
      await PreventivoForm.saveForm(el, prev?.id);
    });

    if (isNew) {
      el.querySelector('#f-data')?.addEventListener('change', (e) => {
        const scadenzaInput = el.querySelector('#f-scadenza');
        if (scadenzaInput && e.target.value) scadenzaInput.value = addDays(e.target.value, 7);
      });
    }
    ...
    bindCapAutoFill(
      el.querySelector('#f-ccap'),
      el.querySelector('#f-ccitta'),
      el.querySelector('#f-cprov'),
      el.querySelector('#f-cnaz')
    );

    bindPivaCfValidator(
      el.querySelector('#f-cpiva'),
      el.querySelector('#f-ccf')
    );

    if (isNew) {
      // Auto-save logic
      const draftKey = 'draft_preventivo';
      
      const saveDraft = () => {
        const data = PreventivoForm.getFormData(el);
        if (data.titolo || data.cliente_nome) {
          localStorage.setItem(draftKey, JSON.stringify(data));
        }
      };

      el.addEventListener('input', () => {
        if (PreventivoForm._draftTimer) clearTimeout(PreventivoForm._draftTimer);
        PreventivoForm._draftTimer = setTimeout(saveDraft, 1000);
      });
      ...
      if (draftStr) {
        try {
          ...
          if (confirm('Hai una bozza non salvata per un nuovo preventivo. Vuoi ripristinarla?')) {
            el.querySelector('#f-titolo').value = draft.titolo || '';
            el.querySelector('#f-cnome').value = draft.cliente_nome || '';
            el.querySelector('#f-crs').value = draft.cliente_ragione_sociale || '';
            el.querySelector('#f-cpiva').value = draft.cliente_piva || '';
            el.querySelector('#f-ccf').value = draft.cliente_cf || '';
            el.querySelector('#f-cemail').value = draft.cliente_email || '';
            el.querySelector('#f-ctel').value = draft.cliente_telefono || '';
            el.querySelector('#f-cind').value = draft.cliente_indirizzo || '';
            el.querySelector('#f-ccitta').value = draft.cliente_citta || '';
            el.querySelector('#f-ccap').value = draft.cliente_cap || '';
            el.querySelector('#f-cprov').value = draft.cliente_provincia || '';
            el.querySelector('#f-cnaz').value = draft.cliente_nazione || 'IT';
            el.querySelector('#f-csdi').value = draft.cliente_codice_destinatario || '';
            el.querySelector('#f-cpec').value = draft.cliente_pec || '';
            el.querySelector('#f-cpa').checked = !!draft.cliente_pa;
            el.querySelector('#f-notecl').value = draft.note_cliente || '';
            el.querySelector('#f-notein').value = draft.note_interne || '';
            el.querySelector('#f-condpag').value = draft.condizioni_pagamento || '';
          } else {
  ...
  getFormData(el) {
    return {
      ...
      cliente_nome: el.querySelector('#f-cnome')?.value.trim(),
      ...
      cliente_provincia: el.querySelector('#f-cprov')?.value || '',
      cliente_nazione: el.querySelector('#f-cnaz')?.value || 'IT',
      cliente_codice_destinatario: el.querySelector('#f-csdi')?.value || '',
      cliente_pec: el.querySelector('#f-cpec')?.value || '',
      cliente_pa: el.querySelector('#f-cpa')?.checked ? 1 : 0,
  ...
  async saveForm(el, id) {
    ...
    try {
      ...
      if (!id) {
        ...
        newId = res.id;
        localStorage.removeItem('draft_preventivo'); // Clear draft on successful save
