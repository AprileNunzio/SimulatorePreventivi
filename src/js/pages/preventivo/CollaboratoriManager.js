...
export default {
  async renderCollaboratori(el, assegnazioni, prevId) {
    ...
    // ── Raggruppa per titolo_voce: somma prezzo_al_cliente e compenso_calcolato ──
    const gruppi = {};
    assegnazioni.forEach(a => {
      ...
      if (!gruppi[titolo]) {
        gruppi[titolo] = {
          ...
          assegnazioni: [], // Mantiene le assegnazioni originali per modifica/elimina
        };
      }
    ...
    collabList.innerHTML = Object.values(gruppi).map((g, gIdx) => {
      ...
      const a = g.assegnazioni[0]; // Per accedere ai dati del collaboratore singolo

      // Se c'è 1 solo collaboratore nel gruppo, mostriamo info complete
      // Se più collaboratori stessa voce, mostriamo voce aggregata
      ...
      // Bottoni: se singolo mostra modifica, se multiplo mostra espandi
