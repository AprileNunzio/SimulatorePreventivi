export class OllamaProvider {
    ...
    async generate(messages, tools) {
        const payload = {
            ...
            options: {
                temperature: 0.1,
                // Con molti tool registrati (schemi + descrizioni) il solo prompt di sistema può avvicinarsi
                // o superare i 4096 token di contesto di default di Ollama, causando risposte troncate o
                // errori del server sui turni successivi. Alziamo il contesto per lasciare margine.
                num_ctx: 8192
            }
        };
        ...
        if (!response.ok) {
            let details = '';
            try { details = (await response.text()).slice(0, 300); } catch (e) {}
            throw new Error(`Ollama HTTP Error: ${response.status}${details ? ' — ' + details : ''}`);
        }
