...
function createAssegnazione(data) {
  ...
  run(`
    ...
    (preventivo_id, collaboratore_id, tipo_compenso, compenso_fisso, percentuale_applicata, compenso_calcolato, titolo_voce, prezzo_al_cliente, note, uuid)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `, [
...
function searchCollaboratori(query) {
  if (!query) return [];
  const q = `%${query.toLowerCase()}%`;
  return all(
    'SELECT * FROM collaboratori WHERE LOWER(nome) LIKE ? OR LOWER(cognome) LIKE ? ORDER BY nome ASC LIMIT 50', 
    [q, q]
  );
}
