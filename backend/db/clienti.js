...
function upsertCliente(data) {
  // Try to find if client already exists by P.IVA or Name+RagioneSociale
  let existing = null;
  ...
  if (existing) {
    ...
    run(`
      UPDATE clienti SET 
        nome = ?, ragione_sociale = ?, piva = ?, cf = ?, email = ?, telefono = ?, indirizzo = ?, citta = ?, cap = ?, updated_at = datetime('now')
      WHERE id = ?
    `, [
    ...
    // Insert
    run(`
      INSERT INTO clienti (nome, ragione_sociale, piva, cf, email, telefono, indirizzo, citta, cap)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    `, [
      data.nome || '', data.ragione_sociale || '', data.piva || '', data.cf || '', 
      data.email || '', data.telefono || '', data.indirizzo || '', data.citta || '', data.cap || ''
...
function getAllClienti() {
  return all('SELECT * FROM clienti ORDER BY nome ASC');
}
