...
function getDataPath() {
  const permanentPath = path.join(app.getPath('appData'), 'NunzioTech', 'Simulatore_Preventivi');
  ...
  if (!fs.existsSync(permanentDbPath)) {
    ...
    const possibleOldPaths = [
      ...
      path.join(exeDir, 'data', 'database.db'),
      path.join(app.getPath('desktop'), 'Simulatore_Preventivi', 'data', 'database.db'),
    ...
    for (const oldPath of possibleOldPaths) {
      if (fs.existsSync(oldPath)) {
        try {
          console.log(`[MIGRAZIONE LEGACY] Trovato DB in: ${oldPath}`);
          fs.copyFileSync(oldPath, permanentDbPath);
          break;
        } catch (err) {
          console.error(`[MIGRAZIONE LEGACY] Errore copia:`, err);
        }
      }
    }
  }
...
const docPath = path.join(app.getPath('documents'), 'NunzioTech', 'Simulatore_Preventivi');
global.EXPORTS_PATH = path.join(docPath, 'exports');
